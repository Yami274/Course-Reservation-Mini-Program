  ...
  <scroll-view scroll-y class="tab-body" :style="{ height: bodyH + 'px' }">
      ...
      <view class="user-card card">
        ...
        <view class="user-row">
          ...
          <view style="flex:1;min-width:0;">
            ...
            <text class="user-sub">注册于 {{ joinDate }} · 已上 {{ doneCount }} 节</text>
          </view>
          <SvgIcon name="chevron" color="#8A7E70" :size="28" />
        </view>
        <!-- 统计数据 -->
        <view class="stats-row">
          <view v-for="stat in stats" :key="stat.label" class="stat-item" @tap="stat.action">
    ...
    <view style="padding: 24rpx 32rpx 0;" v-if="nextLesson">
      <view class="next-lesson-card card">
        ...
        <view style="flex:1;min-width:0;">
          <text class="next-title">{{ nextLesson.title }}</text>
          <text class="next-sub">{{ nextLesson.time }}</text>
        </view>
        <view class="nav-btn" @tap="toLocation">导航</view>
      </view>
    </view>
    ...
    <view style="padding: 24rpx 32rpx 0;">
      <view class="menu-card card">
        <view v-for="(item, i) in mainMenuItems" :key="item.key" class="menu-item" :class="{ last: i === mainMenuItems.length - 1 }" @tap="item.action">
          <SvgIcon :name="item.icon" :color="item.color" :size="44" :filled="item.filled" />
          <text class="menu-label">{{ item.label }}</text>
          <view v-if="item.badge" class="notif-dot" />
    ...
    <view style="padding: 16rpx 32rpx 0;">
      <view class="menu-card card">
        <view v-for="(item, i) in contactMenuItems" :key="item.key" class="menu-item" :class="{ last: i === contactMenuItems.length - 1 }" @tap="item.action">
          <SvgIcon :name="item.icon" :color="item.color" :size="44" />
          <text class="menu-label">{{ item.label }}</text>
          <text v-if="item.sub" class="menu-sub">{{ item.sub }}</text>
          <SvgIcon name="chevron" color="#8A7E70" :size="28" />
        </view>
...
onMounted(async () => {
  ...
  if (!userStore.isLoggedIn) { try { await userStore.login() } catch (e) { /* skip */ } }
  await loadStats()
})
...
async function loadStats() {
  try {
    const [studentsRes, ordersRes] = await Promise.all([
      getStudents().catch(() => []),
      getOrders({ page: 1, pageSize: 50 }).catch(() => ({ list: [] })),
    ])
    ...
    // Nearest upcoming approved order
    const now = new Date()
    const upcoming = orders
      .filter(o => o.status === 'approved')
      .sort((a, b) => (a.schedule?.date || '').localeCompare(b.schedule?.date || ''))
      .find(o => o.schedule?.date && new Date(o.schedule.date) >= now)
...
const joinDate = computed(() => {
  ...
  return new Date(u.created_at).toLocaleDateString('zh', { year: 'numeric', month: '2-digit' }).replace(/\//g, '.')
})

const stats = computed(() => [
  { label: '学员档案', value: studentCount.value, action: () => uni.navigateTo({ url: '/pages/mine/students' }) },
  { label: '已收藏', value: favoriteCount.value, action: () => uni.navigateTo({ url: '/pages/mine/favorites' }) },
  { label: '进行中', value: approvedCount.value, action: () => emit('switchTab', 2) },
  { label: '已完成', value: doneCount.value, action: () => emit('switchTab', 2) },
])

const mainMenuItems = computed(() => [
  {
    key: 'students', label: '学员档案', sub: studentCount.value + ' 位',
    icon: 'user', color: '#D97757',
    action: () => uni.navigateTo({ url: '/pages/mine/students' }),
  },
  {
    key: 'favorites', label: '我的收藏', sub: favoriteCount.value > 0 ? String(favoriteCount.value) : '',
    icon: 'heart', color: '#A0473F', filled: true,
    action: () => uni.navigateTo({ url: '/pages/mine/favorites' }),
  },
  {
    key: 'notifications', label: '系统通知', sub: notificationBadge.value > 0 ? notificationBadge.value + ' 条新' : '',
    icon: 'bell', color: '#E8B860',
    badge: notificationBadge.value > 0,
    action: () => uni.navigateTo({ url: '/pages/mine/notifications' }),
  },
  {
    key: 'location', label: '上课地点', sub: '查看导航',
    icon: 'pin', color: '#6B7F5A',
...
const contactMenuItems = computed(() => [
  {
    key: 'wechat', label: '联系客服', sub: '在线',
    icon: 'wechat', color: '#6B7F5A',
    action: () => uni.showToast({ title: '请扫码联系客服', icon: 'none' }),
  },
  {
    key: 'phone', label: '拨打电话', sub: '0571-8888',
    icon: 'phone', color: '#D97757',
    action: () => uni.makePhoneCall({ phoneNumber: '0571-8888-2026' }),
  },
  {
    key: 'about', label: '关于画室', sub: '',
    icon: 'book', color: '#5A4F44',
...
.notif-dot {
  ...
  box-shadow: 0 0 0 4rpx var(--card);
  flex-shrink: 0;
}
